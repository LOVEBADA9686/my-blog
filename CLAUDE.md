...
- `write.html` + `js/write.js`: 새 글 작성 및 수정 폼. `js/github-api.js`를 통해 **GitHub Contents API를 브라우저에서 직접 호출**해 저장소에 커밋한다 (백엔드가 없으므로 이 사이트에서 "진짜로 게시"할 수 있는 유일한 방법).
- GitHub Personal Access Token은 사용자가 `write.html`에서 직접 입력하며, `localStorage`(`myblog_gh_token` 키)에만 저장된다. 코드에 토큰을 하드코딩하지 않는다.
- 브라우저에서 작성한 글은 `manifest.json` 항목에 `passwordHash`(SHA-256, Web Crypto API)를 추가로 가진다. 이 값으로 수정/삭제 시 비밀번호를 검증한다 (`post.html`의 수정/삭제 버튼 → `js/post.js`).
  - 주의: `passwordHash`는 공개 저장소에 그대로 노출되므로 진짜 보안 장치가 아니라 가벼운 확인 절차에 가깝다. 실질적인 쓰기 권한은 GitHub 토큰을 가진 브라우저만 갖는다.
- `passwordHash`가 없는 글(예: 예약 로봇이 자동 작성한 글, 초기 시드 글)은 브라우저 UI로 수정/삭제할 수 없다.
- GitHub Pages는 커밋 이후 재배포에 최대 1분 정도 걸리므로, 저장/삭제 직후 바로 새로고침해도 반영이 안 보일 수 있다.
...
- 브라우저에서 글 작성/수정/삭제 (`write.html`, GitHub 토큰 필요 — 위 "브라우저에서 글 작성/수정/삭제" 참고)
